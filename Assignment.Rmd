---
title: "Report to predict the manner of exercising"
author: "Rajae"
date: "7/2/2020"
output:
...
We load the training and testing set from the online sources and split the training set into training and test sets. 
...
Once the dataset is clean, we will look for the dependencies between variables through a correlation plot. 
...
In the plot above, darker gradient correspond to having high correlation. We run a Principal Component Analysis to reduce the correlated variables but we aren't doing that due to the number of correlations being quite few.

## Prediction Model Selection

We will use 3 methods to model the training set and thereby choose the one having the best accuracy to predict the outcome variable in the testing set. We will use Decision Tree, Random Forest and Generalized Boosted Model.

Besides, we plot a confusion matrix at the end of each model in order to visualize the analysis better.
...
As Random Forest offers the maximum accuracy of 99.75%, we will use the Random Forest Model to predict our test data class variable.
